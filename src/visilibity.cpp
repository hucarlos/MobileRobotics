...
namespace VisiLibity
{
  ...
  Point Point::projection_onto(const Line_Segment& line_segment_temp) const
  {
    ...
    //(1-theta)*line_segment_temp.second.  if theta is outside
    //the interval [0,1], then one of the Line_Segment's endpoints_
    //must be closest to point_temp.
    double theta = 
	...
	*(line_segment_temp.second().y()
	...
	+ (1-theta)*line_segment_temp.second();
 ...
 Point Point::projection_onto(const Polyline& polyline_temp) const
   ...
   //Trivial case.
    ...
    while( third < vertices_.size() ){
      if(   distance(  Line_Segment( (*this)[first], 
		       ...
		       (*this)[second]  ) 
	    <= epsilon   ){
	//second is redundant, so skip it.
	second = third;
	third++;
      }
      else{
	//second is not redundant, so add it.
  ...
  bool Polygon::is_in_standard_form() const
  {
	...
	if(vertices_[0] > vertices_[i]) {
	  std::cerr<< "--- Test polygon " << vertices_[0]  << " " << vertices_[i] << std::endl;
	  return false;
	}
      }
    return true;
  ...
  void Polygon::eliminate_redundant_vertices(double epsilon)
  {
    ...
    //Enforcing standard form ensures the first vertex is not redundant.
  ...
  bool equivalent(Polygon polygon1, Polygon polygon2, double epsilon)
  {
    ...
    if( polygon1.n() != polygon2.n() )
      return false;
    for(unsigned i=0; i<polygon1.n(); i++)
      if( distance(polygon1[i], polygon2[i]) > epsilon )
	return false;
    return true;
  }
  ...
  bool Environment::is_in_standard_form() const
  {
	...
	or outer_boundary_.area() < 0 ) {
      std::cerr << "--- Test environ "  << outer_boundary_.area() <<  std::endl;      
      return false;
    }
    for(unsigned i=0; i<holes_.size(); i++)
      if( holes_[i].is_in_standard_form() == false 
	  or holes_[i].area() > 0 ) {
	std::cerr << "--- Test area " << holes_[i].area() << std::endl;
	return false;
      }
    return true;
  }
  ...
  bool Environment::is_valid(double epsilon) const
  {
    ...
    //Check all Polygons are in standard form (lex. smallest vertex
    //first in list).
    if( !outer_boundary_.is_in_standard_form() ){
      std::cerr << std::endl << "\x1b[31m" 
		<< "The outer boundary is not in standard form." 
		<<  "\x1b[0m" << std::endl;
      return false;
    }
    for(unsigned i=0; i<h(); i++)
      if( !holes_[i].is_in_standard_form() ){
	std::cerr << std::endl << "\x1b[31m" 
		  << "Hole " << i << " is not in standard form." 
		  <<  "\x1b[0m" << std::endl;
	return false;
      }
	...
	for(unsigned j=0; j<holes_[i].n(); j++){ 
	  std::cerr << " (" << holes_[i][j].x() << "," << holes_[i][j].y()<< " )"  << std::endl;
	}
	std::cerr << "Outer boundary" << std::endl;
	for(unsigned j=0; j<outer_boundary_.n(); j++){ 
	  std::cerr << " (" << outer_boundary_[j].x() << "," << outer_boundary_[j].y()<< " )"  << std::endl;
	}
	return false; 
      }
    for(unsigned i=0; i<h(); i++)
	...
	if( !holes_[i][j].in(outer_boundary_, epsilon) ){
	  std::cerr << std::endl << "\x1b[31m" 
		    << "Vertex " << j << " (" << holes_[i][j].x() << "," << holes_[i][j].y()
		    << ") of hole " << i 
					 ...
					 double epsilon)
  {
	...
	if(PRINTING_DEBUG_DATA){
      ...
      //The Visibility_Polygon should have a minimal representation and
      //be in standard form.
      //enforce_standard_form();
				     ...
				     double epsilon)
  {
    ...
    //allocate a contiguous chunk of memory for adjacenct_matrix_
