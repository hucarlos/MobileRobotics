...
namespace VisiLibity
{
  ...
  /** \brief  get a uniform random sample from an interval on the real line
   *
   ...
   * \remarks  Uses the Standard Library's rand() function.
   */  
  ...
  public:
    ...
    Point(const double& x_temp, const double& y_temp)
    { x_=x_temp; y_=y_temp; }
    //Accessors
    /// get x coordinate
     ...
     * \pre  the calling Point data are numbers and \a line_segment_temp 
     * is nonempty
     * \return  true iff the calling Point is within distance \a epsilon
     * of \line_segment_temp, but distance greater than epsilon from 
     * \a line_segment_temp 's endpoints.
     */
   ...
   *
   * \remarks  NAN==NAN returns false, so if two points have 
  ...
  /** \brief  true iff endpoint coordinates are equal,
  ...
  /** \brief  true iff line segments' endpoints match up w/in an epsilon ball
   *          of each other, false if either line segment has size 0
   *
   * \author  Karl J. Obermeyer
   * \pre  \a line_segment1.size() > 0 and \a line_segment2.size() > 0
   * \remarks  respects ordering of vertices, i.e., even if the line segments 
   * overlap exactly, they are not considered equivalent unless the orientations
   * are the same
   */
   ...
   * line_segment2 is greater than epsilon. size 1 results if the
   * segments intersect poperly, form a T intersection, or --
  ...
  /// add angles' radians and mod int [0, 2*M_PI)
  Angle operator +  (const Angle& angle1, const Angle& angle2);
  /// subtract angles' radians and mod int [0, 2*M_PI)
  Angle operator -  (const Angle& angle1, const Angle& angle2);
   ...
   *
   * \remarks  false if any member data have not been assigned (numbers)
   */
  bool operator >  (const Polar_Point& polar_point1,
			   const Polar_Point& polar_point2);
  /** \brief  compare according to polar lexicographic order 
  ...
  public:
    ...
    { vertices_ = vertices_temp; }
    /** \brief eliminates (\a epsilon) redundant vertices
     *
     * \author  Karl J. Obermeyer
    ...
    //Reduces number of vertices in representation...
    //void smooth(const double& epsilon);
   ...
   *
   * Simple here means no crossing edges or colocated vertices.
  ...
  public:
     ...
     *  
     * \author  Karl J. Obermeyer
     * Works regardless of polygon orientation (ccw vs cw), but
     * assumes no redundant vertices.
     * \remarks  time complexity O(n), where n is the number of
     ...
     *
     * \author  Karl J. Obermeyer
     * A Polygon is considered \a epsilon simple iff (i) the Euclidean
     * distance between nonadjacent edges is no greater than \a
     * epsilon, (ii) adjacent edges intersect only at their single
     * shared Point, and (iii) it has at least 3 vertices.
     */
    ...
    std::vector<Point> random_points(const unsigned& count,
				     double epsilon=0.0) const;
    /** \brief  write list of vertices to file 
     *
     * \author  Karl J. Obermeyer
  ...
  /** \brief  true iff vertex lists are identical
   *
  ...
  bool operator != (Polygon polygon1, Polygon polygon2);
  /** \brief true iff the Polygon's vertices match up w/in an epsilon
   *         ball of each other
   *
   * Respects number, ordering, and orientation of vertices, i.e.,
   * even if the (conceptual) polygons represented by two Polygons are
   * idetical, they are not considered \a epsilon equivalent unless the
   * number of vertices is the same, the orientations are the same, and
   * the Points of the vertex lists match up within epsilon (without
   * permuting them to obtain a match).
   * \author  Karl J. Obermeyer 
  ...
  public:
     ...
     *
     * \a epsilon valid means (i) the outer boundary and hole
     * boundaries are pairwise disjoint with \a epsilon clearance,
     * (ii) holes (including interiors) are pairwise disjoint,
     * (iii) ccw oriented outer boundary, cw oriented holes, and (iv)
     * each vertex list begins with respective lexicographically
     * smallest vertex.
     * \author  Karl J. Obermeyer
     ...
     * epsilon), e.g., returning an std::vector<Polyline>)
     */
    ...
    Polyline shortest_path(const Point& start,
			   const Point& finish,
			   double epsilon=0.0);
    /** \brief  write lists of vertices to file 
     *
     * uses intuitive human and computer readable decimal format with
     * display precision \a fios_precision_temp
     * \author  Karl J. Obermeyer
    ...
    /// clear holes
    void clear_holes()
    { holes_.clear(); update_flattened_index_key(); }
     ...
     */
    void eliminate_redundant_vertices(double epsilon=0.0);
    /// reverse order of vertices belonging to holes only
    void reverse_holes();
  private:    
  ...
  public:
    ...
    Guards(const std::vector<Point>& positions_temp)
     ...
     */
    Bounding_Box bbox() const;
    /** \brief  write list of positions to file 
     *
     * Uses intuitive human and computer readable decimal format with
     * display precision \a fios_precision_temp.
     * \author  Karl J. Obermeyer
    ...
    void add_guard(const Point& point_temp)
  ...
  public:
     ...
     * method Point::snap_to_boundary_of() followed by (order is important)
     * Point::snap_to_vertices_of().
     *
  ...
  /** \brief  visibility graph of an Environment represented
  ...
  public:
    //Constructors
    /** \brief  construct the visibility graph of an Environment
     *
     * \author  Karl J. Obermeyer
  ...
  private:
    ...
    // n_-by-n_ adjacency matrix data stored as 2D dyanmic array
